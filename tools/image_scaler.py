...
def image_scaler(image_path: Union[str, List[str], Image.Image, List[Image.Image]], 
    ...
    - 标准化的输出字典，包含：
        {
            ...
            "data": {
                "primary": List[str],          # 主要输出：缩放后的图片路径列表
                "secondary": Dict,              # 次要输出：详细信息
                "counts": Dict                  # 统计信息
            },
            "metadata": {
                "tool_name": "image_scaler",
                "version": "1.0.0",
    ...
    # Validate parameters
    if scale_factor <= 0:
        error_msg = "Scale factor must be greater than 0."
    ...
    try:
        ...
        logger.info(f"Processing {len(images_to_process)} images with scale factor {scale_factor}")
        ...
        for i, img_input in enumerate(images_to_process):
            try:
                ...
                temp_path = os.path.join(temp_dir, f"scaled_image_{i}_{int(time.time())}.png")
                ...
                logger.info(f"Scaled image {i+1}/{len(images_to_process)}: {original_size} -> {new_size}")
        ...
        output = create_standardized_output(
            tool_name="image_scaler",
            status="success",
            primary_data=temp_paths,  # 主要输出：缩放后的图片路径列表
            secondary_data={
                "results": results,
            ...
            message=f"Processed {successful_count}/{len(images_to_process)} images with scale factor {scale_factor}",
            parameters=parameters,
            start_time=start_time
        )
...
def image_scaler_directory(image_directory: Union[str, List[str]], scale_factor: float = 2.0) -> Dict[str, Any]:
    """
    缩放目录中的所有图片或指定的图片列表
    ...
    - 标准化的输出字典，包含：
        {
            ...
            "metadata": {
                "tool_name": "image_scaler_directory",
                "version": "1.0.0",
                "parameters": Dict,             # 输入参数
                "processing_time": float        # 处理时间
            },
    ...
    try:
        ...
        if isinstance(image_directory, list):
            ...
            if image_directory.strip().startswith('[') and image_directory.strip().endswith(']'):
                try:
                    # 尝试安全解析字符串表示的列表
                    import ast
                    parsed = ast.literal_eval(image_directory)
                    if isinstance(parsed, list):
                        image_files = parsed
                        input_type = "parsed_list"
                        logger.info(f"Parsed {len(image_files)} images from string list")
                    else:
                        raise ValueError("Parsed result is not a list")
                except (ValueError, SyntaxError) as e:
                    logger.error(f"Failed to parse string list: {e}")
                    raise ValueError(f"Invalid list format: {image_directory}")
            # 如果输入是字符串，检查是目录还是单个文件
                ...
                raise ValueError(f"The path '{image_directory}' does not exist or is not accessible.")
        else:
            raise ValueError(f"Invalid input type: {type(image_directory)}. Expected str or List[str], got: {image_directory}")
        ...
        for i, img_path in enumerate(image_files):
            try:
                ...
                logger.info(f"Scaled image {i+1}/{len(image_files)}: {original_size} -> {new_size}")
        ...
        output = create_standardized_output(
            ...
            paths=temp_paths,  # 文件路径列表
            message=f"Processed {successful_count}/{len(image_files)} images with scale factor {scale_factor}",
            parameters=parameters,
            start_time=start_time
        )
