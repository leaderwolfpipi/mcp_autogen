...
def image_generator(style = "anime", text = "$process_node.output.processed_text"):
    """
    通用工具函数 - image_generator
    
    参数说明:
        style: anime
    text: $process_node.output.processed_text
    ...
    try:
        # 参数验证
        logger.info(f"开始执行工具: image_generator")
        ...
        logger.info(f"工具执行完成: image_generator")
